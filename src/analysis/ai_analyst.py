...
置信度（conviction）判断规则——不能和方向强度混淆：
conviction 反映的是"这个判断有多可靠"，不是"预期涨跌幅度有多大"。一个温和但数据完整、
逻辑自洽的信号，置信度可以比一个数据残缺、多空冲突严重的暴涨预期更高。以下任一情况
出现，conviction 最高只能给"中"，不得给"高"：
- 新闻/基本面数据缺失或明显滞后（如近3天无重要新闻、财报数据大量N/A）
- 技术面与基本面方向冲突，且你自己也拿不准哪个更可信
- 历史预测记录显示该股近期同类信号胜率低或样本互相矛盾
- 财报窗口临近（7天内），技术面可靠性下降

新闻/催化剂评估框架（预期差法）：
不要只看一条新闻是利好还是利空，要判断"剩余有效预期差"：
  剩余有效预期差 = 实际发生的信息 − 市场原有预期 − 当前价格已经反映的程度
公司/行业消息好不等于股票现在还值得买入——如果价格已经因为这条消息或类似预期提前
动过（看近期涨跌幅和成交量是否已经计入），剩余的上涨空间可能已经很有限。评估每条
重要新闻时要判断：这是意外信息还是市场早有预期？是否是过去几天已经反复出现的旧
消息（重复信息不该被当作新催化剂重新计入判断）？"""
...
{{
  ...
  "invalidation": "<失效条件：什么价格位置或什么情况发生，说明这次判断的核心逻辑不再成立；
    不等同于止损价——止损是资金管理纪律，这个是判断本身的前提被推翻，例如"跌破60日均线"
    "催化剂被证伪""财报不及预期"等，中性方向也要填，说明什么情况会让你转向>",
  "news_read": {{
    "has_material_news": true或false（近3天是否有实质性新闻，无关新闻/纯噪音算false）,
    "event_type": "<事件类型：财报|并购|监管|供应链|宏观|产品|无重大新闻，没有就填null>",
    "surprise": "高|中|低|无（相对市场原有预期的意外程度）",
    "priced_in": "已充分反映|部分反映|尚未反映|不适用（当前价格对这条消息的消化程度）",
    "is_repeat": true或false（是否是过去几天已反复出现、不该重新计入判断的旧消息）
  }}
}}
"""
